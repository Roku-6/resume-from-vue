...
export default function ResumeCard(props: Resume) {
  ...
  const deleteResume = ((event: React.MouseEvent<HTMLElement>) => {
    axios.delete<Resume[]>("https://vast-harbor-14767.herokuapp.com/api/resume/" + props.resume_id).then(() => {
      setDeleteConfirm(!deleteConfirm);
    });
    console.log(event);
    event.stopPropagation();
  ...
  const updateResume = (event: React.MouseEvent<HTMLElement>) => {
    axios.put<Resume[]>("https://vast-harbor-14767.herokuapp.com/api/resume/" + props.resume_id, {
      title: title,
